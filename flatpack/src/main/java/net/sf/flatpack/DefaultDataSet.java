...
public class DefaultDataSet implements DataSet {
	...
	private String getStringValue(final String column) {
		...
		return row.getValue(ParserUtils.getColumnIndex(row.getMdkey(),
				metaData, column, parser.isColumnNamesCaseSensitive()));
	}
	...
	public void setValue(final String column, final String value) {
		final Row row = (Row) rows.get(pointer);
		final int colIndex = ParserUtils.getColumnIndex(row.getMdkey(),
				metaData, column, parser.isColumnNamesCaseSensitive());
	...
	public boolean isAnError(final int lineNo) {
		for (int i = 0; i < errors.size(); i++) {
			if (((DataError) errors.get(i)).getLineNo() == lineNo
					&& ((DataError) errors.get(i)).getErrorLevel() > 1) {
				return true;
			}
		}
		return false;
	...
	public boolean next() {
		if (pointer < rows.size() && pointer + 1 != rows.size()) {
			pointer++;
			currentRecord = new RowRecord((Row) rows.get(pointer), metaData,
					parser.isColumnNamesCaseSensitive(), pzConvertProps,
					strictNumericParse, upperCase, lowerCase,
					parser.isNullEmptyStrings());
			return true;
		}
		currentRecord = null;
		return false;
	...
	public void absolute(final int localPointer) {
		if (localPointer < 0 || localPointer > rows.size() - 1) {
			throw new IndexOutOfBoundsException("INVALID POINTER LOCATION: "
					+ localPointer);
		}
	...
	public boolean contains(final String column) {
		if (pointer == -1) {
			throw new IndexOutOfBoundsException(
					"dataset on invalid row. need to call next()");
		}
	...
	public boolean isRowEmpty() {
		if (!parser.isFlagEmptyRows()) {
			...
			// throw an exception
			throw new FPInvalidUsageException(
					"Parser.isFlagEmptyRows(true) must be set before using isRowEmpty()");
		}

		if (pointer < 0) {
			throw new FPException(
					"Cursor on invalid row..  Make sure next() is called and returns true");
		}
	...
	public String getRawData() {
		if (!parser.isStoreRawDataToDataSet()) {
			...
			// throw an exception
			throw new FPInvalidUsageException(
					"Parser.isStoreRawDataToDataSet(true) must be set before using getRawData()");
		}
